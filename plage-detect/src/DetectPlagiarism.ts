...
export default class DetectPlagiarism {
		...
		this.submission1.forEach((file1, index1) => {
			...
			this.submission2.forEach((file2, index2) => {
				...
				file1.forEach((node1) => {
					...
					if (
						_.has(node1, "body") ||
						_.has(node1, "expression") ||
						_.has(node1, "arguments") ||
						_.has(node1, "init") ||
						_.has(node1, "declarations")
					) {
						...
						file2.forEach((node2) => {
							...
							if (
								_.has(node2, "body") ||
								_.has(node2, "expression") ||
								_.has(node2, "arguments") ||
								_.has(node2, "init") ||
								_.has(node2, "declarations")
							) {
								//if plagiarism is found
								if (this.compare(node1, node2) === true) {
									//add lines to sets of file 1
									for (
										let i = node1.loc.start.line;
	...
	private compare(obj1, obj2): boolean {
		...
		if (
			obj1 != null &&
			obj2 != null &&
			obj1.type === "BinaryExpression" &&
			obj2.type === "BinaryExpression"
		) {
			return (
				(this.compare(obj1.left, obj2.left) &&
					this.compare(obj1.right, obj2.right)) ||
				(this.compare(obj1.right, obj2.left) &&
					this.compare(obj1.left, obj2.right))
			)
		}
